...
argument-hint: "[--background|--wait] [--resume|--fresh] [--model <model|fast>] [--effort <none|minimal|low|medium|high|xhigh>] [what Grok Build should investigate, solve, or continue]"
...
- Otherwise, before starting Grok Build, check for a resumable rescue thread from this Claude session by running:
...
- If the user did not supply a request, ask what Grok Build should investigate or fix.
