...
| A Grok task rejects `session/new` and lists Codex `thread/*` or `turn/*` methods | Plugin 0.2.1 or earlier connected to a Codex broker after shared `CLAUDE_PLUGIN_DATA` was overwritten by another plugin hook | Update to 0.2.2 or later and start a new session. The plugin now isolates its data path, rejects foreign brokers, and clears stale foreign endpoint records automatically. |
